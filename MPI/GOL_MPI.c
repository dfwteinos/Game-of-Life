...
void GOL_Simulation(int iterations, int rowsPerProc, int columnsPerProc, char ***previousBlock, char ***currentBlock, MPI_Request **sendingArray, MPI_Request **receivingArray, MPI_Comm CartesianCommunicator)
{
  ...
  for(int iteration = 0; iteration < iterations; iteration++)
  {
    ...
    updateInternalBlock(rowsPerProc, columnsPerProc, previousBlock, currentBlock);
...
void updateInternalBlock(int rowsPerProc, int columnsPerProc, char ***previousBlock, char ***currentBlock)
{
  int curr_row, curr_column, active_neighbours;
